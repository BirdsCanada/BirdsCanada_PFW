...
dat9<-fread("Data/PFW_Canada_2022-2023.csv")
dat10<-fread("Data/PFW_Canada_2023-2024.csv")
#New data
dat11<-fread("Data/PFW_Canada_2024-2025.csv")
...
dat9<-dat9 %>% dplyr::select(-alt_full_spp_code, -user_id, -observer_id, -housing_density, -no_birds)
dat10<-dat10 %>% dplyr::select(-alt_full_spp_code, -user_id, -observer_id, -housing_density, -no_birds)
dat11<-dat11 %>% dplyr::select(-alt_full_spp_code)
```
...
data<-rbind(dat1, dat2, dat3, dat4, dat5, dat6, dat7, dat8, dat9, dat10, dat11)
...
First, lets create the zero-fill range file for each Province by creating a master list that links all `loc_id` from PFW to Province, the we determine if a species was detected >=10 times. This is done using a loop, which creates a Range output table. Note that we use the `REPORT_AS` for the species ID field, *not* `species_code` (which would at first glance seems to make sense, but in the long run it doesn't work out. Trust me!).
