...
master<-dat %>% dplyr::select(statprov_code) %>% distinct() 

sp.list<-unique(dat$SpeciesCode) #n=256 in full dataset. 
...
for(m in 1:length(sp.list)) {
 ...
 # m<-1 #for testing each species
  ...
  sp.data <-NULL 
  sp.data <- filter(dat, SpeciesCode == sp.list[m]) %>%
      droplevels()

# Count number of observation in each Atlas Block  
  sp.data<-sp.data %>% group_by(statprov_code) %>%
...
master<-left_join(master, sp.data, by = "statprov_code" )
...
event.data <- data %>%
  dplyr::select(SurveyAreaIdentifier, SamplingEventIdentifier, latitude, longitude, survey_month, survey_day, survey_year, Period, floor.week, Effort_days, Effort_hrs, statprov_code, region) %>%
  group_by(SurveyAreaIdentifier, SamplingEventIdentifier, latitude, longitude, survey_month, survey_day, survey_year, Period, floor.week, Effort_days, Effort_hrs, statprov_code, region) %>%
  distinct() %>%
  ungroup() %>%
  as.data.frame()
