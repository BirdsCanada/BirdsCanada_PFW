...
loc_id_reg<-event %>% group_by(region, Period) %>% summarize (n_loc = n_distinct(loc_id)) %>% drop_na()
...
loc_id_prov<-event %>% group_by(Prov, Period) %>% summarize (n_loc = n_distinct(loc_id)) %>%  drop_na()
...
loc_id_nat<-event %>% group_by(Period) %>% summarize (n_loc = n_distinct(loc_id)) %>% drop_na()
...
ggplot(loc_id_nat, aes(x=Period, y=n_loc))+
  ...
  annotate("text", x = 2012, y = 2200, label = "COVID-19", colour="red", size = 5)

ggsave(paste(out.dir, "nat_eff.pdf", sep=""), width = 11, height = 8, device = "pdf")
while (!is.null(dev.list()))  dev.off()

write.table(loc_id_reg, file = paste(out.dir,"Site Effort_reg.csv"), row.names = FALSE, col.name = TRUE, append = FALSE, quote = FALSE, sep = ",")

write.table(loc_id_prov, file = paste(out.dir,"Site Effort_prov.csv"), row.names = FALSE, col.name = TRUE, append = FALSE, quote = FALSE, sep = ",")

write.table(loc_id_nat, file = paste(out.dir,"Site Effort_nat.csv"), row.names = FALSE, col.name = TRUE, append = FALSE, quote = FALSE, sep = ",")
```
...
days_reg<-event %>% group_by(region, Period) %>% summarize (n_days = sum(Effort_days)) %>% drop_na() %>% filter(Period>=2014)
...
days_prov<-event %>% group_by(Prov, Period) %>% summarize (n_days = sum(Effort_days)) %>% drop_na() %>% filter(Period>=2014)
...
days_nat<-event %>% group_by(Period) %>% summarize (n_days = sum(Effort_days)) %>% drop_na() %>% filter(Period>=2014)
