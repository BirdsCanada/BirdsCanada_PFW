...
events<-data %>% select(loc_id, year, month, day, snow_dep_atleast, housing_density) %>% distinct()
...
HOSP$week <- as.integer(factor(HOSP$floor.week))
sp.data<-dplyr::left_join(events, HOSP, by=c("loc_id", "day", "month", "year"), multiple = "all")
...
HOSP<-data %>% filter(species_code=="houspa") %>% distinct()
HOSP$week <- as.integer(factor(HOSP$floor.week))
sp.data<-dplyr::left_join(events, HOSP, by=c("loc_id", "day", "month", "year", "housing_density"), multiple = "all")
sp.data<-sp.data %>% filter(loc_id != "NA") 
