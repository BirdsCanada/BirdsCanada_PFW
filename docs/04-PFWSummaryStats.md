...
This summary statistic captures the mean group size of species for sites and year when it is present. It is therefore not a zero-filled data matrix. 


```r
# Region
mean.sp<-data %>% group_by(Period, region, REPORT_AS) %>% 
  summarise(mean.group=mean(how_many))
...
# Provincial 
mean.sp<-data %>% group_by(Period, Prov, REPORT_AS) %>% 
  summarise(mean.group=mean(how_many))
...
# National
mean.sp<-data %>% group_by(Period, REPORT_AS) %>% 
  summarise(mean.group=mean(how_many))
...
<img src="04-PFWSummaryStats_files/figure-html/spmean plot-1.png" width="672" />
...
<img src="04-PFWSummaryStats_files/figure-html/spmean plot-2.png" width="672" />
